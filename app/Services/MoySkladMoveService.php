...
class MoySkladMoveService
{
    ...
    public function getMove($moveId)
    {
        ...
        try {
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $this->token,
                'Accept-Encoding' => 'gzip',
            ])->get($this->baseUrl . '/entity/move/' . $moveId);
            ...
            Log::error('Ошибка получения информации о перемещении', [
                'move_id' => $moveId,
                'error' => $e->getMessage()
            ]);
            return null;
     ...
     */
    public function getMoves($limit = 50, $offset = 0)
    {
        if (!$this->hasCredentials()) {
            return null;
        }
        ...
        try {
            ...
            Log::error('Ошибка получения перемещений', [
                'error' => $e->getMessage()
            ]);
            return null;
        }
