...
{
    $worker = Worker::create(['name' => 'Мастер Тестов', 'positions' => ['Мастер']]);
...
test('User::isMaster() возвращает false для должности Работник', function () {
    $worker = Worker::create(['name' => 'Пильщиков', 'positions' => ['Работник']]);
...
test('мастер после входа попадает на журнал приёмок', function () {
    $worker = Worker::create(['name' => 'Мастер Входов', 'positions' => ['Мастер']]);
    User::factory()->create([
        'phone'     => '79991110001',
        'is_admin'  => false,
        'worker_id' => $worker->id,
...
test('рабочий после входа попадает на дашборд, а не на журнал приёмок', function () {
    $worker = Worker::create(['name' => 'Пильщик Входов', 'positions' => ['Работник']]);
    User::factory()->create([
        'phone'     => '79991110002',
        'is_admin'  => false,
        'worker_id' => $worker->id,
...
test('мастер не может открыть главную страницу — редирект на дашборд мастера', function () {
    $user = makeMasterUser();
...
test('мастер может открыть список работников', function () {
    $user = makeMasterUser();
...
test('рабочий не ограничен middleware мастера (его ограничивает WorkerOnly)', function () {
    $worker = Worker::create(['name' => 'Пильщиков Тест', 'positions' => ['Работник']]);
...
test('мастер может использовать AJAX-эндпоинт партий работника', function () {
    $cutter = Worker::create(['name' => 'Пильщик AJAX', 'positions' => ['Работник']]);
    $user   = makeMasterUser();
