...
test('User::isMaster() возвращает false для должности Пильщик', function () {
    $worker = Worker::create(['name' => 'Пильщиков', 'positions' => ['Пильщик']]);
...
test('рабочий после входа попадает на дашборд, а не на журнал приёмок', function () {
    $worker = Worker::create(['name' => 'Пильщик Входов', 'positions' => ['Пильщик']]);
    User::factory()->create([
        'phone'     => '79991110002',
        'is_admin'  => false,
        'worker_id' => $worker->id,
...
test('рабочий не ограничен middleware мастера (его ограничивает WorkerOnly)', function () {
    $worker = Worker::create(['name' => 'Пильщиков Тест', 'positions' => ['Пильщик']]);
...
test('мастер может использовать AJAX-эндпоинт партий работника', function () {
    $cutter = Worker::create(['name' => 'Пильщик AJAX', 'positions' => ['Пильщик']]);
    $user   = makeMasterUser();
